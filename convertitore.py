...
def compare_numeric_columns(df1, df2):
    # Make sure df1 and df2 have the same index
...
if uploaded_file1 and input_date1:
    ...
    if not processed_data1.empty:
        ...
        csv1 = processed_data1.to_csv().encode('utf-8')
        st.download_button("Download processed data as CSV for the first file", data=csv1, file_name='processed_pivot_data1.csv', mime='text/csv')
...
if uploaded_file2 and input_date2:
    ...
    if not processed_data2.empty:
        ...
        csv2 = processed_data2.to_csv().encode('utf-8')
        st.download_button("Download processed data as CSV for the second file", data=csv2, file_name='processed_pivot_data2.csv', mime='text/csv')
